...
export default function VerificationPage() {
    ...
    const handleSendOtp = () => {
        ...
        setLoading(true);
        if (timerRefs.current.otpDelay) clearTimeout(timerRefs.current.otpDelay);
        timerRefs.current.otpDelay = setTimeout(() => {
            setLoading(false);
            setStep(2);
            // Start resend timer
            if (timerRefs.current.resendInterval) clearInterval(timerRefs.current.resendInterval);
            timerRefs.current.resendInterval = setInterval(() => {
                setResendTimer((prev) => {
                    if (prev <= 1) {
                        if (timerRefs.current.resendInterval) clearInterval(timerRefs.current.resendInterval);
                        return 0;
