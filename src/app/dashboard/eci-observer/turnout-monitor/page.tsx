...
const specialVoters = [
    ...
    { category: "First-time Voters (18-19)", registered: 185400, voted: 142458, turnout: 76.8, icon: "school", color: "#4ade80" },
    { category: "Service Voters", registered: 12680, voted: 9764, turnout: 77.0, icon: "military_tech", color: "#fbbf24" },
    { category: "NRI Voters", registered: 3420, voted: 1539, turnout: 45.0, icon: "flight", color: "#60a5fa" },
    { category: "Third Gender", registered: 890, voted: 383, turnout: 43.0, icon: "diversity_3", color: "#f87171" },
];

const statusColors: Record<string, string> = { "ON TRACK": "#4ade80", "ABOVE AVG": "#60a5fa", "BELOW AVG": "#fbbf24", CRITICAL: "#f87171" };
const boothStatusColor: Record<string, string> = { NORMAL: "#4ade80", "HIGH TRAFFIC": "#fbbf24", "LOW TURNOUT": "#f87171", CRITICAL: "#f87171", "QUEUE BUILDUP": "#fbbf24" };
...
export default function TurnoutMonitorPage() {
    ...
    return (
        <ECIPageLayout title="Voter Turnout Monitor" badge="⏱ LIVE TRACKING" badgeColor="#4ade80"
            actions={
                <div className="flex items-center gap-3">
                    ...
                    <span className="text-[9px] font-mono text-white/30">Closes: 6:00 PM IST</span>
                </div>
            }
        >
            <div className="grid grid-cols-5 gap-3 mb-6">
                <ECIKPI icon="people" label="Total Eligible" value="6.15 Cr" sub="543 constituencies" color="#60a5fa" delay={0} />
                <ECIKPI icon="how_to_vote" label="Votes Cast" value="3.87 Cr" sub="62.9% turnout" color="#4ade80" delay={0.06} />
                ...
                <ECIKPI icon="speed" label="Voting Rate" value="48K/hr" sub="↑ 12% vs last" color="#fbbf24" delay={0.24} />
            </div>
                ...
                <ECICard>
                            ...
                            <div className="flex gap-1">
                                {(["overall", "gender", "area"] as const).map(m => (
                                    <button key={m} onClick={() => setChartMode(m)}
                                        className={`text-[8px] font-mono px-2.5 py-1 rounded transition-all capitalize ${chartMode === m ? "bg-red-500/15 text-red-400" : "text-white/25 hover:text-white/50"}`}
                                    >{m}</button>
                                ))}
                            </div>
                        }
                    ...
                    <div className="p-5">
                        <div className="flex items-end gap-2 h-52">
                            {hourlyData.map((d, i) => (
                                <div key={d.hour} className="flex-1 flex flex-col items-center gap-1">
                                    <span className="text-[8px] font-mono text-white/50 font-medium">{d.pct}%</span>
                                    {chartMode === "overall" && (
                                        ...
                                        >
                                            <div className="absolute -top-8 left-1/2 -translate-x-1/2 bg-[#1a1f2e] px-2 py-1 rounded text-[7px] font-mono text-white/80 whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity border border-white/10 z-10">
                                    ...
                                    <span className="text-[7px] font-mono text-white/25">{d.hour}</span>
                                </div>
                            ))}
                        </div>
                        {/* Legend */}
                        <div className="flex items-center gap-4 mt-3 pt-3 border-t border-white/[0.04]">
                            {chartMode === "gender" && <>
                                <div className="flex items-center gap-1.5"><div className="w-3 h-2 bg-blue-400 rounded-sm" /><span className="text-[9px] text-white/35">Male</span></div>
                                <div className="flex items-center gap-1.5"><div className="w-3 h-2 bg-pink-400 rounded-sm" /><span className="text-[9px] text-white/35">Female</span></div>
                            </>}
                            {chartMode === "area" && <>
                                <div className="flex items-center gap-1.5"><div className="w-3 h-2 bg-cyan-400 rounded-sm" /><span className="text-[9px] text-white/35">Urban</span></div>
                                <div className="flex items-center gap-1.5"><div className="w-3 h-2 bg-amber-400 rounded-sm" /><span className="text-[9px] text-white/35">Rural</span></div>
                            </>}
                            <div className="flex-1" />
                            <span className="text-[8px] font-mono text-white/20">Target: 67% by close · Previous: 65.8%</span>
                        </div>
                    </div>
                </ECICard>
                ...
                <div className="space-y-4">
                    <ECICard delay={0.15}>
                        ...
                        <div className="p-4 space-y-4">
                            {[
                                ...
                                { label: "Rural Turnout", pct: 72.2, voters: "2.35 Cr", color: "#4ade80", icon: "grass" },
                            ].map((d, i) => (
                                <motion.div key={d.label} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 + i * 0.1 }}>
                                    <div className="flex justify-between items-center mb-1.5">
                                        <div className="flex items-center gap-1.5"><Icon name={d.icon} size={14} style={{ color: d.color }} /><span className="text-[11px] text-white/60">{d.label}</span></div>
                                        <span className="text-[14px] font-bold" style={{ color: d.color }}>{d.pct}%</span>
                                    </div>
                                    <Bar pct={d.pct} color={d.color} h={6} />
                                    <span className="text-[9px] font-mono text-white/25 mt-1 block">{d.voters} votes cast</span>
                                </motion.div>
                            ))}
                        </div>
                    </ECICard>
                    ...
                    <ECICard delay={0.2}>
                        ...
                        <div className="p-4 space-y-3">
                            {[
                                { label: "Current 2026", pct: 62.9, color: "#f87171", active: true },
                                ...
                                <div key={c.label} className="space-y-1">
                                    <div className="flex justify-between text-[10px]">
                                        <span className={c.active ? "text-white/80 font-medium" : "text-white/40"}>{c.label}</span>
                                ...
                                <motion.div key={i} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 + i * 0.06 }}
                                    ...
                                    <div className="flex justify-between mb-0.5">
                                        <span className="text-[8px] font-mono px-1.5 py-0.5 rounded" style={{
                                            color: a.level === "CRITICAL" ? "#f87171" : a.level === "HIGH" ? "#fbbf24" : "#60a5fa",
                                            background: (a.level === "CRITICAL" ? "#f87171" : a.level === "HIGH" ? "#fbbf24" : "#60a5fa") + "12",
                                        }}>{a.level}</span>
                                        <span className="text-[7px] font-mono text-white/15">{a.time}</span>
                                    </div>
                                    <p className="text-[10px] text-white/60">{a.msg}</p>
                                </motion.div>
                            ))}
                        </div>
                    </ECICard>
            ...
            <div className="flex gap-2 mb-4">
                {(["states", "booths", "demographics"] as const).map(t => (
                    <button key={t} onClick={() => setTab(t)}
                        className={`text-[10px] font-mono px-4 py-2 rounded-lg transition-all capitalize flex items-center gap-1.5 ${tab === t ? "bg-red-500/15 text-red-400 border border-red-500/25" : "text-white/30 border border-white/[0.06] hover:text-white/60"}`}
                    >
            ...
            <AnimatePresence mode="wait">
                {tab === "states" && (
                    <motion.div key="states" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>
                        <ECICard>
                            <div>
                                <div className="grid grid-cols-[1fr_65px_65px_65px_90px_85px_70px_80px] gap-2 px-5 py-2.5 border-b border-white/[0.04] text-[8px] font-mono text-white/20 uppercase tracking-[1.5px]">
                                ...
                                {stateWise.map((s, i) => (
                                    <motion.div key={s.state} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.04 }}
                                        ...
                                        className={`grid grid-cols-[1fr_65px_65px_65px_90px_85px_70px_80px] gap-2 px-5 py-3 border-b border-white/[0.03] cursor-pointer transition-all duration-200 items-center ${selectedState === s.state ? "bg-red-500/5" : "hover:bg-white/[0.015]"}`}
                                    >
                                        ...
                                        <div className="flex items-center gap-1">
                                            ...
                                            <span className="text-[10px] font-mono text-white/60">{s.turnout}%</span>
                                        </div>
                                        ...
                                        <span className="text-[10px] text-white/45 font-mono">{(s.voted / 1000000).toFixed(1)}M</span>
                                        <span className="text-[10px] text-white/30 font-mono">{(s.total / 1000000).toFixed(1)}M</span>
                                        <span className="text-[10px] font-mono" style={{ color: s.evmIssues > 0 ? "#f87171" : "#4ade80" }}>{s.evmIssues} issues</span>
                    ...
                    <motion.div key="booths" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>
                        <ECICard>
                            <ECISectionHeader title="Booth-Level Monitoring" icon="location_on"
                                action={<span className="text-[8px] font-mono text-white/25">{boothLevel.length} booths in selected area</span>}
                            />
                            <div className="grid grid-cols-2 gap-3 p-4">
                                {boothLevel.map((b, i) => (
                                    <motion.div key={b.id} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: i * 0.05 }}
                                        className={`p-4 rounded-lg border transition-all hover:scale-[1.01] cursor-pointer ${b.status === "CRITICAL" || b.status === "LOW TURNOUT" ? "bg-red-500/5 border-red-500/15" : b.status === "QUEUE BUILDUP" || b.status === "HIGH TRAFFIC" ? "bg-yellow-500/5 border-yellow-500/10" : "bg-white/[0.02] border-white/[0.05]"}`}
                                    >
                                        <div className="flex justify-between items-start mb-2">
                                            <div>
                                                <span className="text-[12px] text-white/85 font-semibold">{b.id}</span>
                                                <p className="text-[9px] text-white/30 font-mono">{b.area}</p>
                                            </div>
                                        ...
                                        <div className="space-y-1.5 mb-2">
                                            <div className="flex justify-between text-[9px]">
                                                <span className="text-white/35">Turnout</span>
                                                <span className="text-white/70 font-mono font-medium">{b.turnout}%</span>
                                            </div>
                                            <Bar pct={b.turnout} color={b.turnout > 60 ? "#4ade80" : b.turnout > 45 ? "#fbbf24" : "#f87171"} h={5} />
                                        </div>
                                        <div className="flex justify-between text-[9px] text-white/30 font-mono">
                                            <span>{b.cast}/{b.voters} voted</span>
                                            <span className={b.queue > 20 ? "text-yellow-400" : "text-white/30"}>🧍 Queue: {b.queue}</span>
                                        </div>
                                    </motion.div>
                                ))}
                            </div>
                    ...
                    <motion.div key="demo" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>
                        <ECICard>
                            <ECISectionHeader title="Special Category Voter Turnout" icon="diversity_3"
                                action={<span className="text-[8px] font-mono text-white/25">As per ECI Form 17C</span>}
                            />
                                ...
                                {specialVoters.map((v, i) => (
                                    ...
                                    >
                                        <div className="flex items-center gap-2 mb-3">
                                            ...
                                            <span className="text-[11px] text-white/70 font-medium leading-tight">{v.category}</span>
                                        </div>
                                        <div className="mb-2">
                                            <span className="text-[28px] font-bold text-white/90">{v.turnout}%</span>
                                        </div>
                                        <Bar pct={v.turnout} color={v.color} h={5} />
                                        <div className="flex justify-between text-[8px] font-mono text-white/25 mt-2">
