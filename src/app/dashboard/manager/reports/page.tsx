...
const misData = [
    { metric: "Overall Booth Coverage", value: "73%", trend: "+4%", period: "This Week", color: "#4ade80" },
    { metric: "Worker Utilization", value: "89%", trend: "+2%", period: "Today", color: "#c9a84c" },
    ...
    { metric: "Event Mobilization", value: "72%", trend: "-3%", period: "Last Event", color: "#fbbf24" },
];

const typeColor: Record<string, string> = { Auto: "#4ade80", AI: "#818cf8", Manual: "#fbbf24" };
    ...
    return (
        <ManagerPageLayout title="Reports & MIS" badge="📊 ANALYTICS" badgeColor="#60a5fa">
            <div className="grid grid-cols-5 gap-3">
                ...
                <MgrKPI icon="schedule" label="Auto Reports" value={String(reports.filter(r => r.type === "Auto").length)} sub="Scheduled" color="#4ade80" delay={0.1} />
                <MgrKPI icon="download" label="Grievances" value={stats?.totalGrievances?.toString() || "342"} sub={`${stats?.unresolvedGrievances || 0} unresolved`} color="#c9a84c" delay={0.15} />
                <MgrKPI icon="trending_up" label="Total Voters" value={stats?.totalVoters ? `${(stats.totalVoters / 1000).toFixed(1)}K` : "73%"} sub="In system" color="#4ade80" delay={0.2} />
            </div>

            <div className="flex gap-2 mb-1">
                {(["reports", "mis"] as const).map(t => (
                    <button key={t} onClick={() => setTab(t)} className={`text-[10px] font-mono px-4 py-2 rounded-lg capitalize transition-all flex items-center gap-1.5 ${tab === t ? "bg-[#c9a84c]/15 text-[#c9a84c] border border-[#c9a84c]/25" : "text-white/25 border border-white/[0.06] hover:text-white/50"}`}>
            ...
            {tab === "reports" && (
                <MgrCard delay={0.1}>
                    <MgrSection title="Available Reports" icon="folder" action={
                        <button onClick={() => toast("Custom report builder coming soon", "info")} className="text-[8px] font-mono px-3 py-1 rounded bg-[#c9a84c]/12 text-[#c9a84c] border border-[#c9a84c]/20 flex items-center gap-1"><Icon name="add" size={11} /> Custom Report</button>
                    } />
                    {reports.map((r, i) => (
                        <motion.div key={r.id} initial={{ opacity: 0, x: -6 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.04 }}
                            className="px-5 py-4 border-b border-white/[0.03] hover:bg-white/[0.015] transition-colors cursor-pointer flex items-center gap-4">
                            ...
                            <div className="flex-1 min-w-0">
                                <h4 className="text-[12px] text-white/75 font-medium">{r.title}</h4>
                                <div className="flex gap-3 text-[8px] text-white/25 mt-0.5">
                            ...
                            <div className="flex gap-1.5 shrink-0">
                                <button onClick={() => handleGenerate(r.title)} className="text-[8px] font-mono px-2.5 py-1 rounded bg-[#c9a84c]/12 text-[#c9a84c] border border-[#c9a84c]/20 hover:bg-[#c9a84c]/20 transition-all">GENERATE</button>
                                <button onClick={() => toast(`Downloading ${r.title} (${r.format})...`, "info")} className="text-[8px] font-mono px-2.5 py-1 rounded bg-white/5 text-white/30 border border-white/10 hover:bg-white/10 transition-all">DOWNLOAD</button>
                            </div>
                        </motion.div>
                    ))}
                </MgrCard>
                ...
                <div className="grid grid-cols-3 gap-4">
                    {misData.map((m, i) => (
                        <motion.div key={m.metric} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: i * 0.05 }}>
                            <MgrCard>
                                <div className="p-5">
                                    <p className="text-[9px] font-mono text-white/25 uppercase tracking-wider mb-2">{m.metric}</p>
                                    ...
                                    <p className="text-[8px] font-mono text-white/20">{m.period}</p>
