...
export default function ECIPageLayout({ title, badge, badgeColor = "#f87171", children, actions }: ECILayoutProps) {
    ...
    return (
        <div className="flex h-screen bg-[#06080e] text-white overflow-hidden relative">
            {isSidebarOpen && (
                <div
            ...
            <main className="flex-1 overflow-y-auto">
                <motion.header
                    ...
                    className="sticky top-0 z-10 bg-[#06080e]/80 backdrop-blur-xl border-b border-white/[0.06] px-4 sm:px-6 py-4 flex items-center justify-between"
                >
                    <div className="flex items-center gap-3">
                        <button className="md:hidden text-white" onClick={() => setIsSidebarOpen(!isSidebarOpen)}>
                            <Icon name="menu" size={24} />
                        </button>
                        <h1 className="font-serif text-lg sm:text-xl font-bold text-white tracking-tight">{title}</h1>
                        {badge && (
                            <motion.span
                    ...
                    {actions && <div className="flex flex-col sm:flex-row items-end sm:items-center gap-1 sm:gap-3">{actions}</div>}
                </motion.header>
